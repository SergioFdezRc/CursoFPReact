import React from "react";

const DeleteIcon = () => (
    <svg xmlns="http://www.w3.org/2000/svg" width="1em" height="1em" viewBox="0 0 24 24">
        <path fill="currentColor"
              d="M7.615 20q-.67 0-1.143-.472Q6 19.056 6 18.385V6H5V5h4v-.77h6V5h4v1h-1v12.385q0 .69-.462 1.152q-.463.463-1.153.463zM17 6H7v12.385q0 .269.173.442t.442.173h8.77q.23 0 .423-.192q.192-.193.192-.423zM9.808 17h1V8h-1zm3.384 0h1V8h-1zM7 6v13z"/>
    </svg>
);

const User = ({id, item, onDelete}) => {
    const [isHovered, setIsHovered] = React.useState(false);
    return (
        <div key={id} className="user-item" onMouseEnter={() => setIsHovered(true)} onMouseLeave={() => setIsHovered(false)}>
            <img src={item?.picture?.large} alt={item?.name?.first}/>
            <div className="item-info">
                <h2>{item?.name?.title} {item?.name?.first}</h2>
                <p>{item?.name?.last}</p>
            </div>
            {isHovered && <button className="round-btn" onClick={() => onDelete(id)}><DeleteIcon/></button>}
        </div>
    )
};

export default User;